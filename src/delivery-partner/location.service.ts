...
export class LocationService {
  ...
  async updateDeliveryPartnerLocation(
    ...
    try {
      ...
      const locationRepo = await this.tenantRepos.getRepository(
  ...
  async getLatestLocation(orderId: string): Promise<{
    latitude: number;
    longitude: number;
    timestamp: Date;
    isStale: boolean;
    ...
    try {
      ...
      const location = await locationRepo.findOne({
        where: tenantId
          ? tenantWhere(tenantId, { orderId }, dedicated)
          : { orderId },
        order: { createdAt: 'DESC' },
      });
  ...
  async getOptimizedRoute(orderId: string): Promise<RouteDataDto> {
    try {
      ...
      const order = await orderRepo.findOne({
        where: tenantId
          ? tenantWhere(tenantId, { id: orderId }, dedicated)
          : { id: orderId },
      });
      if (!order) {
        return {

...
import { UpdateLocationDto, LiveLocationResponseDto, RouteDataDto, LocationResponseDto } from '../dto/location.dto';
...
export class DeliveryPartnerController {
    ...
    async getProfile(@CurrentUser() user: any): Promise<DeliveryPartnerApiResponseDto<DeliveryPartnerProfileResponseDto>> {
    ...
    @ApiQuery({ name: 'date', required: false, description: 'Optional date filter (YYYY-MM-DD), defaults to today', example: '2026-02-26' })
    async getAssignedOrders(
        @CurrentUser() user: any,
        @Query('date') date?: string,
    ): Promise<DeliveryPartnerApiResponseDto<AssignedOrderResponseDto[]>> {
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const orders = await this.deliveryPartnerService.getAssignedOrders(partner.id, date);
    ...
    @ApiOperation({ summary: 'Get in-progress (accepted) orders for the delivery partner' })
    @ApiResponse({
        status: 200,
        description: 'List of in-progress orders',
        type: [AssignedOrderResponseDto],
    })
    async getInProgressOrders(@CurrentUser() user: any): Promise<DeliveryPartnerApiResponseDto<AssignedOrderResponseDto[]>> {
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const orders = await this.deliveryPartnerService.getInProgressOrders(partner.id);
    ...
    @ApiOperation({ summary: 'Get all completed orders for the delivery partner' })
    @ApiResponse({
        status: 200,
    ...
    async getCompletedOrders(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const result = await this.deliveryPartnerService.getCompletedOrders(partner.id, query);
    ...
    @ApiOperation({ summary: 'Accept an assigned order and start delivery (moves to OUT_FOR_DELIVERY)' })
    @ApiParam({ name: 'id', description: 'Order ID' })
    @ApiResponse({ status: 200, description: 'Order accepted and delivery started' })
    ...
    async acceptOrder(
        ...
        @CurrentUser() user: any,
    ): Promise<DeliveryPartnerApiResponseDto<{ orderId: string; status: string }>> {
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const order = await this.deliveryPartnerService.acceptOrder(partner.id, orderId);
    ...
    async updateOrderStatus(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const order = await this.deliveryPartnerService.updateOrderStatus(partner.id, orderId, dto);
    ...
    async uploadDeliveryProof(
        ...
        @CurrentUser() user: any,
    ): Promise<DeliveryPartnerApiResponseDto<{ orderId: string; proofUrl: string }>> {
        if (!file) {
        ...
        try {
            const order = await this.deliveryPartnerService.uploadDeliveryProof(partner.id, orderId, proofUrl);
            ...
            if (error instanceof BadRequestException || error.status === 400 || error.status === 403 || error.status === 404) {
                throw error;
            }
    ...
    async uploadSubscriptionDeliveryProof(
        ...
        @UploadedFile() file: Express.Multer.File,
        @CurrentUser() user: any,
    ): Promise<DeliveryPartnerApiResponseDto<{ subscriptionId: string; proofUrl: string }>> {
        if (!file) {
            throw new BadRequestException('Proof image file is required');
        }
    ...
    async getCustomerDetails(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const customerDetails = await this.deliveryPartnerService.getCustomerDetails(partner.id, customerId);
    ...
    @Get('subscriptions')
    @ApiOperation({ summary: 'Get all subscriptions assigned to the delivery partner' })
    @ApiResponse({
        status: 200,
    ...
    async getAssignedSubscriptions(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const subscriptions = await this.deliveryPartnerService.getAssignedSubscriptions(partner.id, date);
    ...
    @ApiOperation({ summary: 'Get all completed subscriptions for the delivery partner' })
    @ApiResponse({
        status: 200,
    ...
    async getCompletedSubscriptions(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const result = await this.deliveryPartnerService.getCompletedSubscriptions(partner.id, query);
    ...
    async getSubscriptionById(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const subscription = await this.deliveryPartnerService.getSubscriptionById(partner.id, subscriptionId);
    ...
    async updateSubscriptionStatus(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const log = await this.deliveryPartnerService.updateSubscriptionDeliveryStatus(partner.id, subscriptionId, dto);
    ...
    @Get('route')
    @ApiOperation({ summary: 'Get combined route for today (Orders + Subscriptions)' })
    @ApiResponse({
        status: 200,
    ...
    async getDailyRoute(
        ...
        const partner = await this.deliveryPartnerService.getDeliveryPartnerByUserId(user.id);
        const route = await this.deliveryPartnerService.getDailyRoute(partner.id, date);
    ...
    @ApiResponse({ status: 200, description: 'Location updated', type: LocationResponseDto })
    ...
    async updateLocation(
        ...
        try {
            ...
            this.logger.error(`Failed to update location for order ${dto.orderId}:`, error);
            throw error;
        }
    }
    ...
    @ApiResponse({ status: 200, description: 'Live location data', type: LiveLocationResponseDto })
    @ApiResponse({ status: 404, description: 'Order not found' })
    async getLiveLocation(
        @Param('orderId') orderId: string,
    ): Promise<LiveLocationResponseDto> {
        const latestLocation = await this.locationService.getLatestLocation(orderId);
        ...
        return {
            ...
            message: latestLocation.isStale ? 'Location data is older than 5 minutes' : undefined,
        };
    }
    ...
    @ApiOperation({ summary: 'Batch start delivery for multiple orders and subscriptions' })
    @ApiResponse({
        status: 200,
    ...
    async startBatchDelivery(
        ...
        return {
            success: true,
            message: 'Trip started successfully. All items marked as OUT_FOR_DELIVERY.'
        };
    }
}

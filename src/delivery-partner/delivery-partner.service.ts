...
import { SubscriptionDeliveryLog, DeliveryStatus } from '../entities/subscription-delivery-log.entity';
import { User, Role } from '../entities/user.entity';
import { DeliveryPartnerQueryDto } from './dto/delivery-partner-query.dto';
import { applyPagination, PaginationResult } from '../common/utils/pagination.util';
...
export class DeliveryPartnerService {
    ...
    private statusTransitions: Record<OrderStatus, OrderStatus[]> = {
        [OrderStatus.PENDING]: [],
        [OrderStatus.ASSIGNED]: [OrderStatus.OUT_FOR_DELIVERY, OrderStatus.DELIVERED, OrderStatus.FAILED],
     ...
     */
    async getAssignedOrders(deliveryPartnerId: string, date?: string): Promise<AssignedOrderResponseDto[]> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        ...
        let query = orderRepo.createQueryBuilder('order')
            ...
            .where('order.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId })
        ...
        const orders = await query.orderBy('order.createdAt', 'DESC')
            .getMany();
        ...
        return orders.map((order) => ({
            ...
            googleMapsUrl: order.deliveryLatitude && order.deliveryLongitude
     ...
     */
    async getInProgressOrders(deliveryPartnerId: string): Promise<AssignedOrderResponseDto[]> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const orderRepo = await this.tenantRepos.getRepository(Order);
        ...
        const query = orderRepo.createQueryBuilder('order')
            ...
            .leftJoinAndSelect('items.product', 'product')
            .where('order.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId })
            .andWhere('order.status IN (:...statuses)', { statuses: inProgressStatuses });
        applyTenantFilter(query, tenantId, 'order', dedicated);
     ...
     */
    async getCompletedOrders(deliveryPartnerId: string, query: DeliveryPartnerQueryDto): Promise<PaginationResult<AssignedOrderResponseDto>> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const orderRepo = await this.tenantRepos.getRepository(Order);
        ...
        const paginationResult = await applyPagination(queryBuilder, query.page, query.limit);
     ...
     */
    async acceptOrder(deliveryPartnerId: string, orderId: string): Promise<Order> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const orderRepo = await this.tenantRepos.getRepository(Order);
    ...
    async updateOrderStatus(
        ...
        await this.syncSubscriptionLogFromOrder(order, dto.status, deliveryPartnerId, dto.notes, order.deliveryProofUrl, dedicated);
        ...
        if (dto.status === OrderStatus.OUT_FOR_DELIVERY && customer) {
            ...
            }).catch(err => this.logger.warn('Failed to send ORDER_OUT_FOR_DELIVERY notification:', err.message));
        }
        ...
        if (dto.status === OrderStatus.DELIVERED && customer) {
            this.notificationService.sendNotification({
                ...
                channel: 'BOTH',
                fcmToken: customer.fcmToken,
            }).catch(err => this.logger.warn('Failed to send ORDER_DELIVERED notification:', err.message));
        }
    ...
    async uploadDeliveryProof(
        ...
        const allowedStatuses = [OrderStatus.OUT_FOR_DELIVERY, OrderStatus.DELIVERED, OrderStatus.FAILED];
     ...
     */
    async getAssignedSubscriptions(deliveryPartnerId: string, date?: string): Promise<AssignedSubscriptionResponseDto[]> {
        try {
            const tenantId = this.requireTenantId();
            const dedicated = this.tenantContext.usesDedicatedDatabase();
            ...
            const query = subscriptionRepo.createQueryBuilder('subscription')
                ...
                .where('subscription.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId })
                .andWhere('subscription.status = :status', { status: SubscriptionStatus.ACTIVE })
                .andWhere('(subscription.nextDeliveryDate <= :filterDate OR EXISTS (SELECT 1 FROM subscription_delivery_logs log WHERE log.subscriptionId = subscription.id AND log.deliveryDate = :logDate))', {
            ...
            return subscriptions.map((sub) => {
                ...
                const isDeliveredToday = (sub.deliveryLogs || []).some(
                    log => log.deliveryDate === filterDate && log.status === DeliveryStatus.DELIVERED
                );
                ...
                return {
                    ...
                    deliveryLatitude: sub.customer?.latitude ? Number(sub.customer.latitude) : undefined,
                    deliveryLongitude: sub.customer?.longitude ? Number(sub.customer.longitude) : undefined,
                    googleMapsUrl: sub.customer?.latitude && sub.customer?.longitude
            ...
            this.logger.error(`Error in getAssignedSubscriptions: ${error.message}`, error.stack);
            throw new InternalServerErrorException('Failed to fetch assigned subscriptions');
        }
    }
     ...
     */
    async getCompletedSubscriptions(deliveryPartnerId: string, query: DeliveryPartnerQueryDto): Promise<PaginationResult<AssignedSubscriptionResponseDto>> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        ...
        const queryBuilder = deliveryLogRepo.createQueryBuilder('log')
            .leftJoinAndSelect('log.subscription', 'subscription')
            .leftJoinAndSelect('subscription.customer', 'customer')
            .leftJoinAndSelect('subscription.product', 'product')
            .where('log.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId })
        ...
        const admin = await this.userRepo.findOne({ where: tenantWhere(tenantId, { role: Role.ADMIN }, dedicated) });
        ...
        const mappedData = paginationResult.data.map((log) => {
            ...
            return {
                ...
                deliveryLogs: [{ deliveryDate: log.deliveryDate, status: log.status, notes: log.notes }],
                pickupAddress: admin?.address || undefined,
                pickupLatitude: admin?.latitude ? Number(admin.latitude) : undefined,
                pickupLongitude: admin?.longitude ? Number(admin.longitude) : undefined,
                product: {
     ...
     */
    async getSubscriptionById(deliveryPartnerId: string, subscriptionId: string): Promise<AssignedSubscriptionResponseDto> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        const subQb = subscriptionRepo.createQueryBuilder('subscription')
            .leftJoinAndSelect('subscription.customer', 'customer')
            .leftJoinAndSelect('subscription.product', 'product')
            .leftJoinAndSelect('subscription.deliveryLogs', 'deliveryLogs')
            .where('subscription.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId })
        ...
        if (!sub) {
            throw new NotFoundException('Subscription not found or not assigned to you');
        }

        const today = new Date().toISOString().split('T')[0];
        const isDeliveredToday = (sub.deliveryLogs || []).some(
            log => log.deliveryDate === today && log.status === DeliveryStatus.DELIVERED
        );
    ...
    private async syncSubscriptionLogFromOrder(order: Order, status: OrderStatus, deliveryPartnerId: string, notes?: string, proofUrl?: string, dedicated?: boolean) {
        ...
        this.logger.log(`Synced subscription log for sub ${order.subscriptionId} from order ${order.id} status ${status}`);
    }
     ...
     */
    async getDailyRoute(deliveryPartnerId: string, date?: string): Promise<CombinedDeliveryResponseDto[]> {
        // 1. Get Assigned Orders
        const assignedOrders = await this.getAssignedOrders(deliveryPartnerId, date);

        // 2. Get Assigned Subscriptions
        const assignedSubscriptions = await this.getAssignedSubscriptions(deliveryPartnerId, date);
        ...
        assignedSubscriptions.forEach(sub => {
            route.push({
                ...
                type: 'SUBSCRIPTION',
                status: sub.isDeliveredToday ? DeliveryStatus.DELIVERED : DeliveryStatus.PENDING,
    ...
    async registerFCMToken(userId: string, fcmToken: string): Promise<{ success: boolean; message: string }> {
        const tenantId = this.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const deliveryPartnerRepo = await this.tenantRepos.getRepository(DeliveryPartner);
        const partner = await deliveryPartnerRepo.findOne({

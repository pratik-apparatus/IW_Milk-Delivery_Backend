...
export class OrderTrackingResDto {
    ...
    @ApiProperty({ example: 'OUT_FOR_DELIVERY', enum: ['PENDING', 'PACKAGING', 'OUT_FOR_DELIVERY', 'DELIVERED', 'CANCELLED'] })
    status: string;
    ...
    @ApiProperty({ type: LatestLocationDto, nullable: true, description: 'Latest location of delivery partner' })
    latestLocation: LatestLocationDto | null;

    @ApiProperty({ type: DeliveryPartnerInfoDto, nullable: true, description: 'Delivery partner information' })

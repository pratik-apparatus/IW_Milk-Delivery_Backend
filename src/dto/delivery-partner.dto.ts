...
export class UpdateSubscriptionDeliveryStatusDto {
    ...
    @ApiPropertyOptional({ description: 'Optional delivery date (YYYY-MM-DD), defaults to today' })
    @IsOptional()
    @IsString()
...
export class StartBatchDeliveryDto {
    ...
    @ApiProperty({ type: [String], description: 'List of subscription IDs to start' })
    @IsOptional()
    @IsArray()
    @IsString({ each: true })

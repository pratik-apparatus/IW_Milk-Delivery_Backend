...
export class SubscriptionController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Subscription created and wallet debited',
    type: SubscriptionResDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Insufficient balance or invalid plan',
  })
  ...
  @Get()
  @ApiOperation({
    summary: 'Get all my subscriptions with pagination and search',
  })
  @ApiResponse({
    status: 200,
  ...
  async getSubscriptionById(@Param('id') id: string, @CurrentUser() user: any) {
  ...
  @ApiResponse({
    status: 400,
    description: 'Cannot pause non-active subscription',
  })
  @ApiResponse({ status: 404, description: 'Subscription not found' })
  async pauseSubscription(@Param('id') id: string, @CurrentUser() user: any) {
  ...
  @ApiResponse({
    status: 400,
    description: 'Cannot resume non-paused subscription',
  })
  @ApiResponse({ status: 404, description: 'Subscription not found' })
  async resumeSubscription(@Param('id') id: string, @CurrentUser() user: any) {
  ...
  @ApiOperation({
    summary: 'Manually trigger subscription processing (for testing)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 200,
    description: 'Missed delivery added successfully',
    type: SubscriptionResDto,
  })
  @ApiResponse({ status: 404, description: 'Subscription not found' })
  async addMissedDelivery(
    ...
    return await this.subscriptionService.addMissedDelivery(
      user.id,
      subscriptionId,
    );
  }

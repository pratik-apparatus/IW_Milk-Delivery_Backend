...
export class SubscriptionSchedulerService {
    ...
    private async executeSubscriptionProcessing(subscriptionId?: string): Promise<{
        ...
        try {
            ...
            this.logger.log(`Found ${dueSubscriptions.length} subscriptions due for delivery`);
            ...
            for (const subscription of dueSubscriptions) {
                try {
                    const wasPaused = await this.processSubscriptionDelivery(subscription);
            ...
            this.logger.error(`Error in subscription scheduler: ${error.message}`, error.stack);
            return {
    ...
    private async processSubscriptionDelivery(subscription: Subscription): Promise<boolean> {
        const { customerId, product, addressSnapshot, phoneSnapshot } = subscription;
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        ...
        try {
            ...
            const nextDeliveryDate = this.subscriptionService.calculateNextDeliveryDateFromBase(
                subscription,
            ...
            subscription.remainingDeliveries = this.subscriptionService.calculateRemainingDeliveries(
                subscription,
                subscription.nextDeliveryDate || new Date()
            );
    ...
    async triggerSubscriptionProcessing(subscriptionId?: string) {
        this.logger.log(`Manual subscription processing triggered${subscriptionId ? ` for ID ${subscriptionId}` : ''}`);
        ...
        const subscriptionsToFix = await subscriptionRepo.find({
            where: tenantId && !dedicated
                ? { remainingDeliveries: IsNull(), tenantId }
                : { remainingDeliveries: IsNull() },
        });

        if (subscriptionsToFix.length > 0) {
            this.logger.log(`Found ${subscriptionsToFix.length} subscriptions with missing remaining count. Healing data...`);
            const today = new Date();
            for (const sub of subscriptionsToFix) {
                sub.remainingDeliveries = this.subscriptionService.calculateRemainingDeliveries(sub, today);

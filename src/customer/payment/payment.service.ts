...
export class PaymentService {
    ...
    constructor(
        ...
        this.razorpayKeyId = (this.configService.get<string>('RAZORPAY_KEY_ID') || 'rzp_test_S5iJuPus3ggxkS').trim();
        this.razorpayKeySecret = (this.configService.get<string>('RAZORPAY_KEY_SECRET') || 'x1YuOZBSy1g6SL64QNi8C7jL').trim();

        this.logger.log(`Razorpay KeyId loaded: ${this.razorpayKeyId ? this.razorpayKeyId.substring(0, 8) + '...' : 'NOT SET'}`);
    ...
    async createRazorpayOrder(
        ...
        const config = await this.tenantIntegrationConfigService.getRazorpayConfig(
            tenantId,
        );
        ...
        try {
            this.logger.log(`Razorpay KeyId loaded: ${this.razorpayKeyId ? this.razorpayKeyId.substring(0, 8) + '...' : 'NOT SET'}`);
            this.logger.log(`Razorpay KeySecret loaded: ${this.razorpayKeySecret ? this.razorpayKeySecret.substring(0, 8) + '...' : 'NOT SET'}`);
            ...
            this.logger.log(`Payment order created: ${savedPayment.id}, Razorpay Order ID: ${order.id}`);
            ...
            try {
                const errDetails = JSON.stringify(error, Object.getOwnPropertyNames(error));
    ...
    async verifyPayment(
        ...
        const config = await this.tenantIntegrationConfigService.getRazorpayConfig(tenantId);
        const keySecret = config.keySecret || this.razorpayKeySecret;

        this.logger.log(`Verifying payment - Order ID: ${razorpayOrderId}, Payment ID: ${razorpayPaymentId}`);
        ...
        if (!payment) {
            this.logger.error(`Payment record not found for order ID: ${razorpayOrderId}`);
            throw new BadRequestException('Invalid order ID');
        }

        if (payment.customerId !== customerId) {
            this.logger.error(`Order ${razorpayOrderId} does not belong to customer ${customerId}`);
        ...
        if (!isValidSignature) {
            this.logger.error(`Invalid signature for order ${razorpayOrderId}. Expected: ${expectedSignature}, Received: ${razorpaySignature}`);
            ...
            try {
                ...
                this.logger.log(`Payment ${payment.id} marked as FAILED and payment details saved`);
            } catch (saveError) {
        ...
        try {
            await paymentRepo.save(payment);
            this.logger.log(`Payment ${payment.id} verified and saved - Payment ID: ${razorpayPaymentId}`);
        } catch (saveError) {
            this.logger.error(`Failed to save payment record: ${saveError}`);
        ...
        this.logger.log(`Wallet credited for customer ${customerId}. New balance: ${wallet.balance}`);

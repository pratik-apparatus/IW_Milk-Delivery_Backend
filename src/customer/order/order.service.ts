...
export class OrderService {
  ...
  async updateCart(customerId: string, dto: UpdateCartDto) {
    ...
    for (const item of dto.items) {
      ...
      if ((product.remainingQuantity ?? product.quantity) < item.quantity) {
        throw new BadRequestException(
          `Product ${product.name} is not available in this quantity`,
        );
      }
    }
    ...
    try {
      ...
      if (useSavedAddress) {
        if (!customer.address || customer.address.trim() === '') {
          throw new BadRequestException(
            'No saved address found. Please provide a delivery address.',
          );
        }
        ...
        if (!dto.deliveryAddress) {
          throw new BadRequestException(
            'deliveryAddress is required when useSavedAddress is false',
          );
        }
      ...
      for (const item of orderItems) {
        ...
        await queryRunner.manager.decrement(
          Product,
          tenantWhere(tenantId, { id: item.productId }, dedicated),
          'remainingQuantity',
          item.quantity,
        );
      }
      ...
      this.notificationService
        ...
        .catch((err) =>
          this.logger.warn(
            'Failed to send ORDER_PLACED notification:',
            err.message,
          ),
        );
      ...
      if (partner) {
        this.notificationService
          ...
          .catch((err) =>
            this.logger.warn(
              'Failed to send NEW_ORDER_ASSIGNED notification:',
              err.message,
            ),
          );
      }
  ...
  async createOrderFromSubscription(
    ...
    const product = await productRepo.findOne({
      where: tenantId
        ? tenantWhere(tenantId, { id: productId }, dedicated)
        : { id: productId },
    });
    ...
    if (availableStock < quantity) {
      throw new BadRequestException(
        `Insufficient stock for product ${product.name} (ID: ${productId}) for subscription order. Available: ${availableStock}, Requested: ${quantity}.`,
      );
    }
    ...
    try {
      ...
      await queryRunner.manager.decrement(
        Product,
        tenantId
          ? tenantWhere(tenantId, { id: product.id }, dedicated)
          : { id: product.id },
        'remainingQuantity',
        quantity,
      );
      ...
      const partner = await queryRunner.manager.findOne(DeliveryPartner, {
        where: tenantId
          ? tenantWhere(tenantId, { isActive: true }, dedicated)
          : { isActive: true },
        order: {
          CurrentOrder: 'ASC',
          createdAt: 'ASC',
      ...
      this.logger.error(
        `Failed to create subscription order: ${error.message}`,
        error.stack,
      );

...
export class ReportsService {
  ...
  async getOverview(query?: BaseReportQueryDto) {
    ...
    const subscriptionQuery =
      subscriptionRepo.createQueryBuilder('subscription');
    applyTenantFilter(subscriptionQuery, tenantId, 'subscription', dedicated);
    const deliveryPartnerQuery =
      deliveryPartnerRepo.createQueryBuilder('partner');
    ...
    if (startDate) {
      ...
      subscriptionQuery.andWhere('subscription.createdAt >= :startDate', {
        startDate,
      });
    }
    if (endDate) {
      ...
      subscriptionQuery.andWhere('subscription.createdAt <= :endDate', {
        endDate,
      });
    }
    ...
    if (startDate) {
      refundsQuery.andWhere('transaction.createdAt >= :startDate', {
        startDate,
      });
    }
    if (endDate) {
  ...
  async deletePaymentTransactions(paymentId: string) {
    ...
    const payment = await paymentRepo.findOne({
      where: tenantWhere(tenantId, { id: paymentId }, dedicated),
    });
    if (!payment) {
    ...
    const qb = deliveryPartnerRepo
      ...
      .leftJoin(
        ...
        dedicated
          ...
          : 'order.deliveryPartnerId = partner.id AND order.tenantId = :tenantId',
        { tenantId },
      )
    ...
    return {
      data: data.map((item) => ({
        ...
        successRate:
          item.totalOrders > 0
            ? parseFloat(
                ((item.deliveredOrders / item.totalOrders) * 100).toFixed(2),
              )
            : 0,
      })),
      meta: {
  ...
  async clearReports() {
    ...
    const result = await paymentRepo.delete(
      tenantWhere(tenantId, { status: PaymentStatus.FAILED }, dedicated),
    );
    return {
      success: true,

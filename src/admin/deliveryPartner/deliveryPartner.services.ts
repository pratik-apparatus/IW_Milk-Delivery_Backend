...
export class AdminDeliveryPartnerService {
    ...
    private async attachUsers<T extends { userId: string }>(
        ...
        const userIds = [...new Set(partners.map((partner) => partner.userId).filter(Boolean))];
        if (!userIds.length) {
            return partners;
        }
        ...
        qb.andWhere(
            new Brackets((subQb) => {
                ...
                if (matchingUserIds.length) {
                    subQb.orWhere('dp.userId IN (:...matchingUserIds)', { matchingUserIds });
                }
            }),
        );
    }
    ...
    async findAll(query: PaginationQueryDto) {
        ...
        const qb = dpRepo.createQueryBuilder('dp')
    ...
    async findById(id: string) {
        ...
        const orderQb = orderRepo
            ...
            .where('order.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId: partner.id });
        ...
        const subscriptions = await subscriptionRepo.find({
            where: tenantWhere(tenantId, { deliveryPartnerId: partner.id }, dedicated),
    ...
    async update(id: string, dto: CreateDeliveryPartnerDto) {
        ...
        const user = await this.userRepo.findOne({ where: tenantWhere(tenantId, { id: partner.userId }, dedicated) });
        if (user) {
    ...
    async banDeliveryPartner(id: string) {
        ...
        const user = await this.userRepo.findOne({ where: tenantWhere(tenantId, { id: userId }, dedicated) });
        if (user) {
            user.isActive = false;
            await this.userRepo.save(user);
        }

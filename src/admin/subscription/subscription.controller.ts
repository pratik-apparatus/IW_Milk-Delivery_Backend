import { Controller, Get, Query, Delete, Param, Post, Body, Put } from "@nestjs/common";
...
export class subscriptionController {
    ...
    @ApiResponse({ status: 404, description: 'No subscriptions found for this customer' })
    ...
    @ApiResponse({ status: 404, description: 'Subscription or Delivery partner not found' })
    assignDeliveryPartner(
        @Param('id') id: string,
        @Body() assignDto: AssignDeliveryPartnerDto
    ) {
        return this.subscriptionService.assignDeliveryPartner(id, assignDto.deliveryPartnerId);
    }

    @Put(":id/update-delivery-partner")
    @ApiOperation({ summary: 'Update/Edit the assigned delivery partner for a subscription' })
    @ApiParam({ name: 'id', description: 'Subscription ID' })
    @ApiResponse({ status: 200, description: 'Delivery partner updated successfully' })
    @ApiResponse({ status: 404, description: 'Subscription or Delivery partner not found' })
    @ApiResponse({ status: 400, description: 'Cannot assign a banned or inactive delivery partner' })
    updateDeliveryPartner(
        @Param('id') id: string,
        @Body() assignDto: AssignDeliveryPartnerDto
    ) {
        return this.subscriptionService.updateDeliveryPartner(id, assignDto.deliveryPartnerId);
    }
    ...
    @ApiOperation({ summary: 'Manually trigger subscription delivery processing (Data Healing + Order Generation)' })
    @ApiQuery({ name: 'subscriptionId', required: false, description: 'Optional: Process only this specific subscription' })
    @ApiResponse({ status: 200, description: 'Processing triggered successfully' })
    async triggerProcessing(@Query('subscriptionId') subscriptionId?: string) {
        const result = await this.schedulerService.triggerSubscriptionProcessing(subscriptionId);
        return {
            success: true,
            message: subscriptionId ? `Processing completed for subscription ${subscriptionId}` : 'Manual subscription processing completed',
    ...
    @ApiResponse({ status: 200, description: 'Subscription logs deleted successfully' })
    ...
    @ApiOperation({ summary: 'Extend subscription end date due to missed delivery' })
    @ApiParam({ name: 'id', description: 'Subscription ID' })
    ...
    extendMissed(
        ...
        return this.subscriptionService.extendSubscriptionForMissedDelivery(id, dto);
    }
}

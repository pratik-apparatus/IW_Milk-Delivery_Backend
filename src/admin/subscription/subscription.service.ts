...
export class subscriptionService {
    ...
    async getallsub(query: AdminSubscriptionFilterDto) {
        ...
        if (date) {
            qb.andWhere('(subscription.nextDeliveryDate <= :filterDate OR EXISTS (SELECT 1 FROM subscription_delivery_logs log WHERE log.subscriptionId = subscription.id AND log.deliveryDate = :date))', {
                filterDate: new Date(date),
                date
            });
            qb.leftJoinAndSelect('subscription.deliveryLogs', 'deliveryLogs', 'deliveryLogs.deliveryDate = :date', { date });
        }

        if (deliveryPartnerId) {
            qb.andWhere('subscription.deliveryPartnerId = :deliveryPartnerId', { deliveryPartnerId });
        }
    ...
    async getSubscriptionsByCustomerId(customerId: string) {
        ...
        const subscriptions = await subscriptionRepo.find({
            ...
            relations: ['customer', 'product', 'deliveryPartner', 'deliveryLogs', 'orders']
        });
        ...
        const detailedSubscriptions = subscriptions.map(subscription => {
            ...
            const orderedDates = (subscription.orders || [])
                ...
                .map(o => {
                    const d = o.scheduledDeliveryDate ? new Date(o.scheduledDeliveryDate) : new Date(o.createdAt);
            ...
            const missedDates = expectedDates
                .filter(date => {
                    ...
                    return date < today && !loggedDates.includes(dateStr) && !orderedDates.includes(dateStr);
                })
                .map(date => this.formatToYYYYMMDD(date));

            const nextActualDate = expectedDates.find(date => date >= today);
            const remainingDeliveries = expectedDates.filter(date => date >= today).length;

            return {
                ...subscription,
                missedDates,
                autoCalculatedNextDate: nextActualDate ? this.formatToYYYYMMDD(nextActualDate) : null,
                remainingDeliveries,
    ...
    private calculateExpectedDeliveryDates(subscription: Subscription): Date[] {
        ...
        const end = subscription.endDate ? new Date(subscription.endDate) : new Date(start);
        ...
        switch (planType) {
            ...
            case 'WEEKLY':
                if (subscription.selectedDays && subscription.selectedDays.length > 0) {
                    const selectedDayNums = subscription.selectedDays.map(day => dayMap[day]).filter(n => n !== undefined);
                    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    async deleteSubscription(id: string) {
        ...
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        const subscription = await subscriptionRepo.findOne({ where: tenantWhere(tenantId, { id }, dedicated) });
        if (!subscription) {
    ...
    async assignDeliveryPartner(subscriptionId: string, deliveryPartnerId: string) {
        const tenantId = this.tenantContext.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        const deliveryPartnerRepo = await this.tenantRepos.getRepository(DeliveryPartner);
        const subscription = await subscriptionRepo.findOne({ where: tenantWhere(tenantId, { id: subscriptionId }, dedicated) });
        if (!subscription) {
            throw new NotFoundException('Subscription not found');
        }

        const deliveryPartner = await deliveryPartnerRepo.findOne({ where: tenantWhere(tenantId, { id: deliveryPartnerId }, dedicated) });
        if (!deliveryPartner) {
            throw new NotFoundException('Delivery partner not found');
        }

        if (deliveryPartner.isBanned || !deliveryPartner.isActive) {
            throw new BadRequestException('Cannot assign a banned or inactive delivery partner');
        }
    ...
    async updateDeliveryPartner(subscriptionId: string, deliveryPartnerId: string) {
        const tenantId = this.tenantContext.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        const deliveryPartnerRepo = await this.tenantRepos.getRepository(DeliveryPartner);
        ...
        if (subscription.deliveryPartnerId === deliveryPartnerId) {
            throw new BadRequestException(`This subscription is already assigned to delivery partner ID: ${deliveryPartnerId}`);
        }

        const previousDeliveryPartner = subscription.deliveryPartner;

        const newDeliveryPartner = await deliveryPartnerRepo.findOne({ where: tenantWhere(tenantId, { id: deliveryPartnerId }, dedicated) });
    ...
    async getDeliveryHistory(query: PaginationQueryDto, subscriptionId?: string) {
        ...
        const qb = deliveryLogRepo.createQueryBuilder('log')
    ...
    async updateSubscriptionLogStatus(logId: string, dto: UpdateSubscriptionLogStatusDto) {
        const tenantId = this.tenantContext.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const deliveryLogRepo = await this.tenantRepos.getRepository(SubscriptionDeliveryLog);
    ...
    async extendSubscriptionForMissedDelivery(subscriptionId: string, dto: ExtendSubscriptionDto) {
        const tenantId = this.tenantContext.requireTenantId();
        const dedicated = this.tenantContext.usesDedicatedDatabase();
        const subscriptionRepo = await this.tenantRepos.getRepository(Subscription);
        const subscription = await subscriptionRepo.findOne({
        ...
        if (!subscription.endDate || !subscription.startDate) {
            throw new BadRequestException('This subscription does not have a set start or end date to extend');
        }
        ...
        return {
            ...
            data: {
                ...
                nextDeliveryDate: subscription.nextDeliveryDate ? this.formatToYYYYMMDD(subscription.nextDeliveryDate) : null,

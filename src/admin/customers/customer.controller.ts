...
export class CustomerController {
    ...
    @Get()
    @ApiOperation({ summary: "get all customers with subscription details (excludes banned)" })
    @ApiResponse({ status: 200, description: "all customers with their subscriptions and product details", type: [Customer] })
    getAllCustomers(@Query() query: PaginationQueryDto) {
        return this.customerService.getallCustomer(query);
    }

    @Get("all")
    @ApiOperation({ summary: "get all customers with subscription details (alternative route, excludes banned)" })
    @ApiResponse({ status: 200, description: "all customers with their subscriptions and product details", type: [Customer] })
    ...
    @Get("banned")
    @ApiOperation({ summary: "get all banned customers with subscription details" })
    @ApiResponse({ status: 200, description: "all banned customers with their subscriptions and product details", type: [Customer] })
    ...
    @Delete(":id")
    @ApiOperation({ summary: "ban customer and all his data including subscriptions, orders, payments" })
